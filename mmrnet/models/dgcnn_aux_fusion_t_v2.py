# dgcnn_aux_fusion_t_v2.py
# Same architecture as dgcnn_aux_fusion_t but compatible with the new data pipeline:
#   - Accepts (point_cloud, frame_signals) tuple input (frame_signals ignored)
#   - aux_dim=3 by default (Doppler, SNR, Density — Zone removed)
# Use this as a controlled baseline to isolate the effect of frame-level conditioning.
import torch
...
class DGCNNAuxFusionT_V2(nn.Module):
    """
    DGCNN variant with Aux-Fusion inside EdgeConv layers and a temporal transformer.
    V2: compatible with new data pipeline — accepts (point_cloud, frame_signals) tuple,
    ignores frame_signals. aux_dim=3 (Doppler, SNR, Density; Zone removed).
    """
    def __init__(self,
                 info,
                 k=30,
    ...
    def forward(self, data):
        ...
        data: tuple (point_cloud, frame_signals)
          ...
          frame_signals : (B, T, 9)    ignored — no frame-level conditioning in this variant
        returns: logits (B, num_classes)
        """
        point_cloud, _ = data   # frame_signals intentionally unused
        data = point_cloud
        assert data.dim() == 4, "Expect point_cloud shape (B, T, N, C). For single frame use T=1."
        B, T, N, C = data.shape
        device = data.device
