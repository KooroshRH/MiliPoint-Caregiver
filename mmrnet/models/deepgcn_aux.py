...
class DeepGCN_Aux(nn.Module):
    ...
    """
    def __init__(self, info=None, in_channels=7, k=16, num_layers=14, channels=64, aggr='max'):
        super().__init__()
    ...
    def forward(self, data):
        ...
        # Extract position (first 3 channels for k-NN graph construction)
        pos = x[:, :3]

        # Stem: project all features (including auxiliary) to feature space
        x = self.stem(x)
